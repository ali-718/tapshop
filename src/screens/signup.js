   ...
   flexDirection: 'row',
    ...
    return (
        ...
        <View style={style.txtView}>
            <TextInput onChangeText={(val) => {this.setState({Username:val})}} value={this.state.Username} style={style.txtInput} placeholder={this.state.Lang == "en" ? "Username" : "اسم المستخدم"} placeholderTextColor={this.state.grey}></TextInput>
            ...
            <TextInput onChangeText={(val) => {this.setState({Phone:val})}} value={this.state.Phone} style={style.txtInput} placeholder={this.state.Lang == "en" ? "Phone" : "هاتف"} placeholderTextColor={this.state.grey}></TextInput>
            ...
            <TextInput secureTextEntry={true} onChangeText={(val) => {this.setState({ConfirmPassword:val})}} value={this.state.ConfirmPassword} style={style.txtInput} placeholderTextColor={this.state.grey} placeholder={this.state.Lang == "en" ? "Confirm Password" : "تأكيد كلمة المرور"}></TextInput>
        </View>
        <View style={style.loginBtnView}>
            <TouchableOpacity onPress={this.LoginPress} style={style.loginBtn}>
            <Text style={{color:"white", fontWeight:"bold"}}>{this.state.Lang == "en" ? "Register" : "تسجيل"}</Text>
            </TouchableOpacity>
        ...
        <TouchableOpacity onPress={() => this.props.navigation.navigation.navigate('Login')}><Text style={{paddingTop:"3%", fontSize:12,color:"black"}}>{this.state.Lang == "en" ? "Do you have an account" : "هل لديك حساب"} <Text style={{fontWeight:"bold", color:"green"}}>{this.state.Lang == "en" ? "Signin" : "تسجيل الدخول"}</Text></Text></TouchableOpacity>
        </View>
        </View>
        </ScrollView>
